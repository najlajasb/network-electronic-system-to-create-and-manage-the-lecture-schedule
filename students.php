    ...
    <div class="row d-flex justify-content-center">
        <div class="col-xl-12 col-lg-12 col-md-9 col-11 text-center ">
            <div class="card ">
                ...
                <tbody>
                        <?php
                        $results = $pdo->query( 'SELECT users.id, users.name, users.email, colleges.name AS college_name , permissions.name as permission, departments.name as dep_name FROM users JOIN colleges ON users.college_id = colleges.id join permissions on permissions.id = users.permission join departments on departments.id = users.department_id where users.permission = 5');
                        if($results)
                        {
